...
const AddPost = ({ images }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (images.length > 0) {
      console.log("Images from Post Details: ");
      images.map((str, index) => console.log(index, str));
      ...
      try {
        ...
        const postResponse = await axios.post(
          "http://localhost:8000/api/auth/addPost",
          postData
        );
        ...
        await axios.post(
          "http://localhost:8000/api/auth/addPostDetails",
          postDetailData
        );

        console.log("Post and Post Details successfully inserted");
      } catch (error) {
